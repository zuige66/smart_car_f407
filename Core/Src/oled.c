...
void OLED_Send(uint8_t *data, uint8_t len)
{
  ...
  HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(&BOARD_OLED_I2C, OLED_ADDRESS, data, len, 100);
  if (status != HAL_OK) {
    // I2C通信失败，尝试恢复I2C总线
    HAL_I2C_DeInit(&BOARD_OLED_I2C);
    HAL_I2C_Init(&BOARD_OLED_I2C);
  }
